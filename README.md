...
## 概要
Windows 11 向けの軽量な作業メモアプリです。`tkinter` ベースで動作し、タグ選択式の入力欄を最大 10 レーン提供。1クリックで行単位にログへ追記でき、プレビューで最新ログを即確認できます。OpenAI API を用いた LLM 質問エリアも利用できます。

- 実行環境: Windows 11 / Python 3.12
- ログ日付の区切り: 毎日 04:00（午前4時）
- UI: 通常サイズ/拡大サイズのトグル切替
- 10レーン構成: TAG / SUB_TAG / テキスト / 行別「登録」
