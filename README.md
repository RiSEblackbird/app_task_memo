...
- ログエクスプローラー: 本日ログを既定表示、前日/翌日へ移動、日付表示
...
- ログエクスプローラー（新規ウインドウ）
  - 上部に「⇦」「⇨」で日付移動、中央に対象日の表示（例: 《2025年9月7日(日)》）
  - 下部に対象日のログを簡易表示（本日を既定表示。日付境界は 04:00）
- メニュー
...
7. 「ログエクスプローラー」を押すと、別ウインドウで本日のログが開きます。上部の「⇦」「⇨」で前日/翌日に移動し、中央の日付表示（《YYYY年M月D日(曜)》）で確認できます（04:00 を日付境界として扱います）。
